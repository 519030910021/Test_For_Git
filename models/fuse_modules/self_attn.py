...
class AttFusion(nn.Module):
    ...
    def forward(self, x, record_len, delete_list):
        # delete_list: a list of agents need to be deleted
        record_len = record_len - len(delete_list)
        if len(delete_list) > 0:
          index = []
          for i in range(record_len + len(delete_list)):
